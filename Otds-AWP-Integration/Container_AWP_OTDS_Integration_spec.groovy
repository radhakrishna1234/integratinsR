...
//arr[1] =config.getProperty("CSURL"); 
 ...
 def orgPartitionLOcation = otdsConfig.getPartitionLocation(getOrgPartitionURL, method,  orgAdminAuthendPoint, orgAdminAuthPayload)
  ...
  def orgResourceLOcation = otdsConfig.parseResp(orgResourceResp ,"resourceDN");
  
   new File('./','awpresource.txt').withWriter('utf-8') { 
         writer -> writer.writeLine orgResourceLOcation
 }
  
FileInputStream in1 = new FileInputStream(new File("./","config.properties"));
Properties props = new Properties();
props.load(in1);
in1.close();

FileOutputStream out = new FileOutputStream(new File("./","config.properties"));
props.setProperty("appresid", resourceID);
props.setProperty("orgResourceLOcation", orgResourceLOcation);
